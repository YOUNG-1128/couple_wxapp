...
exports.main = async () => {
  try {
    ...
    if (answers.length < 2) {
      throw new Error('answers_not_complete')
    }

    const now = new Date().toISOString()
    const aiResult = await requestAIAnalysis(record.questionText, answers[0].answer, answers[1].answer)
      .catch(() => null)
    const analysisResult = aiResult || buildFallbackAnalysisResult(answers[0].answer, answers[1].answer, record.questionText)
    const aiAnalysis = buildAnalysisText(analysisResult)
    ...
    return {
      success: false,
      errorMessage: (error && (error.message || error.errMsg)) || 'generate_daily_analysis_failed'
    }
  }
}
