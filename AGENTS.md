...
动态列表只会为当前作者本人显示编辑和删除入口。编辑复用动态发布页，但编辑模式不会把已发布动态保存为草稿。`updatePost` 和 `removePost` 都会在服务端同时校验当前用户、情侣关系、动态作者和发布状态，避免修改或删除伴侣发布的动态及其他情侣的数据。
...
- 待办权限规则：个人待办只能由当前用户为自己创建，且仅当前用户可见、可完成、可删除；情侣待办双方可见、双方可完成和删除。页面层不能出现“TA 的个人待办”创建入口，云函数也必须强制校验该规则。
